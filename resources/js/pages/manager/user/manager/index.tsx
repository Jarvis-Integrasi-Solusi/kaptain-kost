...
interface ManagerPageProps extends PageProps {
    managers: User[];
    filters: {
        search?: string;
    };
...
export default function ManagerList() {
    const { managers, filters } = usePage<ManagerPageProps>().props;
    ...
    const { paginatedData, totalPages, totalItems } = useMemo(() => {
        ...
        const filtered = managers.filter((manager) => {
            ...
            return manager.name?.toLowerCase().includes(searchTerm) || manager.email?.toLowerCase().includes(searchTerm);
        });
    ...
    }, [managers, search, currentPage, itemsPerPage]);
    ...
    const handleDelete = async (id: number) => {
        setIsDeleting(true);
        router.delete(`/manager/user/manager/${id}`, {
            onSuccess: () => {
                setDeleteId(null);
            },
            onFinish: () => {
                                    ...
                                    {paginatedData.length === 0 ? (
                                        <TableRow>
                                            <TableCell colSpan={6} className="h-24 text-center">
                                                <div className="flex flex-col items-center gap-2">
                                                    <span>{search ? `No managers found matching "${search}".` : 'No managers found.'}</span>
                                                </div>
                                            </TableCell>
                                        </TableRow>
                                    ) : (
                                        paginatedData.map((manager) => (
                                            <TableRow key={manager.id}>
                                                ...
                                                <TableCell>{manager.created_at ? formatDate(manager.created_at) : '-'}</TableCell>
                                                <TableCell>
                                                    <DropdownMenu>
                                                        ...
                                                        <DropdownMenuContent align="end">
                                                            <DropdownMenuItem asChild>
                                                                <Link href={`/manager/user/manager/${manager.id}`}>
                                                                    <Eye className="mr-2 h-4 w-4" />
                                                                    View
                                                                </Link>
                                                            </DropdownMenuItem>
                                                            <DropdownMenuItem className="text-yellow-600" asChild>
                                                                <Link href={`/manager/user/manager/${manager.id}/edit`}>
                                                                    <Edit className="mr-2 h-4 w-4 text-yellow-600" />
                                                                    Edit
                                                                </Link>
                                                            </DropdownMenuItem>
                                                            <DropdownMenuItem className="text-red-600" onClick={() => setDeleteId(manager.id)}>
                                                            ...
                                                            {manager.status === 'active' && (
                                                                <DropdownMenuItem
                                                                    ...
                                                                    onClick={() => setDeactivateId(manager.id)}
                                                                >
            ...
            <AlertDialog open={!!deactivateId} onOpenChange={() => setDeactivateId(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will deactivate the manager account. The user won’t be able to log in until reactivated.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
