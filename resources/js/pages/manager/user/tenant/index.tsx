...
export default function TenantList() {
    const { tenants, filters } = usePage<TenantPageProps>().props;
    ...
    const { paginatedData, totalPages, totalItems } = useMemo(() => {
        ...
        const filtered = tenants.filter((tenant) => {
            ...
            return (
                ...
                tenant.username?.toLowerCase().includes(searchTerm)
            );
        });
    ...
    }, [tenants, search, currentPage, itemsPerPage]);
    ...
    const handleDelete = async (id: number) => {
        setIsDeleting(true);
        router.delete(`/manager/user/tenant/${id}`, {
            onSuccess: () => {
                setDeleteId(null);
            },
            onFinish: () => {
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription>
                                    Manage your tenant accounts
                                    {search && <span className="ml-2 text-sm text-blue-600">• Showing results for "{search}"</span>}
                                </CardDescription>
                            </div>
                            <Button asChild>
                                <Link href="/manager/user/tenant/create?redirect_to=manager.user.tenant.index">
