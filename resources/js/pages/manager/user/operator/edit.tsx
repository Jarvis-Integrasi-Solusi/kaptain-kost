...
interface OperatorEditPageProps extends PageProps {
    operator: Operator;
    flash?: {
        success?: {
            title: string;
...
export default function EditOperator() {
    const { operator, errors } = usePage<OperatorEditPageProps>().props;
    ...
    // Set initial preview image if operator has existing image
    useEffect(() => {
        if (operator.image && !removeExistingImage) {
            setPreviewImage(`/storage/${operator.image}`);
        } else {
            setPreviewImage(null);
        }
    }, [operator.image, removeExistingImage]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(`/manager/user/operator/${operator.id}`, {
            onSuccess: () => {
                // Don't reset form on edit, just clear password fields
                setData('password', '');
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit Operator - ${operator.name}`} />

            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold">Edit Operator</h1>
                        <p className="text-muted-foreground">Update operator account information</p>
                    </div>
                </div>
                ...
                <div className="grid gap-6 lg:grid-cols-3 lg:items-start">
                    ...
                    <Card className="lg:col-span-2">
                        <CardHeader>
                            ...
                            <CardDescription>Update the operator account details</CardDescription>
                        </CardHeader>
                        <CardContent>
                    ...
                    <Card className="self-start lg:col-span-1">
                        <CardHeader>
                            ...
                            <CardDescription>Update profile image for the operator</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="space-y-2">
                                {previewImage ? (
                                ...
                                {errors.remove_image && <p className="text-sm text-red-500">{errors.remove_image}</p>}
                            </div>
                        </CardContent>
                    </Card>
                </div>
